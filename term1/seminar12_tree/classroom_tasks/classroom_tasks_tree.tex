...
\end{center}
\end{multicols}
\item Написать функцию \texttt{Node* bst\_search(const Node* root, int val)}, которая ищет элемент в бинарном дереве и возвращает указатель на этот элемент. Если такого элемента нет, то функция должна вернуть \texttt{NULL}. Протестируйте эту функцию, печатая поддерево с помощью \texttt{print\_ascii\_tree}.
