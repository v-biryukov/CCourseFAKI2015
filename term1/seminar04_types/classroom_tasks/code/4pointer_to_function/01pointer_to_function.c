/*
    ...
    Как мы знаем, что всё, что мы передаём в функцию копируется (за исключением массивов).
    Указатели, как и обычные переменные, копируются в функции.
    Но что означает копия указателя?
    Если один указатель хранит адрес некоторой переменной, то и его копия будет хранить адрес той же самой переменной.


    Рассмотрим данный пример:

        В функции main создаётся переменная a типа int и указатель p, хранящий адрес a.
        Потом мы передаём p в функцию и он туда копируется.

        Переменная p из функции main и переменная p из функции print_address это разные переменные.
