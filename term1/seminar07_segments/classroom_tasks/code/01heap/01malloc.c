/*
    Функции для динамического выделения памяти содержатся в библиотеки <stdlib.h>
        ...
        void* realloc(void* p, size_t new_n) 

            Перевыделяет выделенную память. Указатель p должен указывать на ранее 
            выделенную память. Память, на которую ранее указывал p, освободится.
            Если память перевыделить не получилось, то функция вернёт значение NULL.
            При этом указатель p будет продолжать указывать на старую память, она не освободится.
