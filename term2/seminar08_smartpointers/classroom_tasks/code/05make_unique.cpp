/*
    ...
    Поэтому лучше использовать специальную функцию std::make_unique, которая будет сама выделять память в куче 
    и создавать объект. Обратите внимание, что в эту функцию передаются аргументы конструктора.
