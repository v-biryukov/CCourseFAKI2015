/*
    ...
    При перемещении одного вектора в другой происходит следующее:
        ...
        2)  Вектор из которого производилось перемещение перестаёт владеть ресурсом (то есть объектами в куче).
            ...
            Каким будет это корректное состояние может различаться у разных объектов.
    ...
    Схематическое изображение памяти после перемещения:
...
int main()
{
    ...
    std::vector<int> b = std::move(a);  // Перемещаем вектор a в вектор b
