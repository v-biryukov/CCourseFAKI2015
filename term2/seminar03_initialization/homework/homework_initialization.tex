...
\subsection{Усечение}
Напишите функцию \texttt{void truncate\_to\_dot(std::string\& s)}, которая будет принимать строку по ссылке и усекать её до первого символа точки. Размер и вместимость строки должны стать как можно более маленькими.
\begin{center}
\begin{tabular}{ l | l }
 до & после \\ \hline
