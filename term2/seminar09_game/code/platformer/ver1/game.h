...
public:
    ...
    Game(int w, int h, std::string* tile_map):
            num_of_tiles_x(h), num_of_tiles_y(w), tile_map(tile_map), screen(0, 0)
    {
        total_time = 0;
    ...
    void update(float time)
    {
        ...
        hero.update(time, tile_map, screen);
        total_time += time;
    }
