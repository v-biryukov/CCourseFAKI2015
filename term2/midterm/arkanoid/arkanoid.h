...
const float ball_radius = 3.f, ball_velocity = 10.f;
...
{
    if (!isIntersecing(mPaddle, mBall)) 
        return false;
...
// Проверяем сталкиваются ли блок и шариком и если они сталкиваются, 
// то задаём соответствующую скорость у шарика
bool testCollision(Brick& mBrick, Ball& mBall) 
{

    if(!isIntersecing(mBrick, mBall)) 
        return false;
